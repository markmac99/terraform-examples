...
variable "reponame" { default = "testwebapprepo" }
variable "containername" { default = "testwebapp" }
...
resource "aws_ecr_repository" "testwebapprepo" {
  name                 = var.reponame
...
resource "aws_s3_object" "codebuild-img-zip-upl" {
  ...
  kms_key_id  = aws_kms_key.container_key.arn
  source_hash = data.archive_file.codebuild-img-zip.output_md5
}

# create the codebuild project 
resource "aws_codebuild_project" "testproject" {
